 ...
 * @author I0360D3
 */
public class Base64EncodedInputStream extends FilterInputStream {
     ...
     */
    public Base64EncodedInputStream(InputStream in, String startString, String endString) {
        super(in);
     ...
     * <code>0</code> to
     * <code>255</code>. If no byte is available because the end of the stream has been reached, the value
     * <code>-1</code> is returned. This method blocks until input data is available, the end of the stream is detected, or
     * an exception is thrown. <p> This method simply performs
    ...
    /**
     * Reads up to
     * <code>len</code> bytes of data from this input stream into an array of bytes. This method blocks until some input is
     * available. <p> This method simply performs
     ...
     * <code>n</code> bytes of data from the input stream. The
     * <code>skip</code> method may, for a variety of reasons, end up skipping over some smaller number of bytes, possibly
     * <code>0</code>. The actual number of bytes skipped is returned. <p> This method simply performs
     * <code>in.skip(n)</code>.
     *
    ...
    /**
     * Returns the number of bytes that can be read from this input stream without blocking. <p> This method simply performs
     * <code>in.available(n)</code> and returns the result.
     *
     ...
     * <code>reset</code> method repositions this stream at the last marked position so that subsequent reads re-read the
     * same bytes. <p> The
     * <code>readlimit</code> argument tells this input stream to allow that many bytes to be read before the mark position
     * gets invalidated. <p> This method simply performs
     ...
     * <code>in.reset()</code>. <p> Stream marks are intended to be used in situations where you need to read ahead a little
     * to see what's in the stream. Often this is most easily done by invoking some general parser. If the stream is of the
     * type handled by the parse, it just chugs along happily. If the stream is not of that type, the parser should toss an
     * exception when it fails. If this happens within readlimit bytes, it allows the outer code to reset the stream and try
     * another parser.
     *
    ...
    private int read1CharAtATimeUntilEndRegionIsFound() {
        //
        ...
        while (indexOf(ba, 0, bytesToCompare, endBytes, 0, endBytes.length, 0) < 0) {
            ...
            if (bytesToCompare >= ba.length) {
                System.out.println("Did not find end , got to end of buffer");
                break;
            }
        }
