...
<xsl:import href="GenericDailyPageLoadTimes.xsl"    />
  ...
  <xsl:template match="Payload" mode="applySecond">
     <xsl:apply-templates select="DailyPageSummary/hibernate-custom" mode="data">
        <xsl:sort select="DailyPageLoadTimBean/averageLoadTime * DailyPageLoadTimBean/totalLoads" data-type="number" order="descending"/>
     </xsl:apply-templates>
  </xsl:template>
</xsl:stylesheet>
