...
<xsl:import href="ViewHistoricalChartsLeftPanel.xsl"    />
  ...
  <xsl:template match="Payload" mode="applyFirst">
     <xsl:apply-templates select="ThirtySecondLoads/ThirtySecondLoadElement" mode="data">
     </xsl:apply-templates>
  </xsl:template>

  <xsl:template match="ThirtySecondLoads/ThirtySecondLoadElement" mode="data">
    <xsl:if test="position() mod 2 =0">
        <tbody class="two">
            <tr>
                ...
                <td class="leftMessage">
                <a>
                    <xsl:attribute name="href">View30SecondLoads.web?page=<xsl:value-of select="PageName"/>&amp;selectedDate=<xsl:value-of select="format-number($Payload/Calendar/CalendarBean/CurrentYear,'00')"/><xsl:value-of select="format-number($Payload/Calendar/CalendarBean/CurrentMonthNumber,'00')"/><xsl:value-of select="format-number($Payload/Calendar/CalendarBean/CurrentDate,'00')"/>
                    </xsl:attribute>
