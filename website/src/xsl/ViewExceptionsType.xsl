    ...
    <xsl:if test="position() mod 2 =0">
        <tbody class="two">
            <tr>
                ...
                <td>
                <a>
                    <xsl:attribute name="href">ViewExceptions.web?eid=<xsl:value-of select="ExceptionTopLine"/>&amp;el=<xsl:value-of select="ExceptionMessage"/>&amp;selectedDate=<xsl:value-of select="format-number($Payload/Calendar/CalendarBean/CurrentYear,'00')"/><xsl:value-of select="format-number($Payload/Calendar/CalendarBean/CurrentMonthNumber,'00')"/><xsl:value-of select="format-number($Payload/Calendar/CalendarBean/CurrentDate,'00')"/>
                    </xsl:attribute>
                    <xsl:attribute name="class">nav</xsl:attribute> 
                    <xsl:value-of select="ExceptionTopLine"/>
                </a>
                </td>
            </tr>
        </tbody>
