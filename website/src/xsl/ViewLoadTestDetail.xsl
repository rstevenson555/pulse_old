...
<xsl:import href="ViewHistoricalChartsLeftPanel.xsl"    />
  ...
  <xsl:template match="Payload" mode="applyFirst">
     <xsl:apply-templates select="LoadTest/LoadTestTransaction" mode="data">
     </xsl:apply-templates>
  </xsl:template>
  <xsl:template match="Payload" mode="applySecond">
     <xsl:apply-templates select="LoadTest/LoadTestSummaryTransaction" mode="data">
     </xsl:apply-templates>
  </xsl:template>

  <xsl:template match="LoadTest/LoadTestSummaryTransaction" mode="data">
...
-->
                ...
                <td>
                    <xsl:value-of select="NinetiethPercentile"/>
                </td>
                <td>
                    <xsl:value-of select="FiftiethPercentile"/>
                </td>
            </tr>
        </tbody>
    </xsl:if>
  ...
  <xsl:template match="LoadTest/LoadTestTransaction" mode="data">
    <xsl:if test="position() mod 2 =0">
        <tbody class="two">
            <tr>
            <!--
...
-->
                ...
                <td>
                    <xsl:value-of select="format-number(average div 1000,'#,###,##0.00')"/>
                </td>
                <td>
                    <xsl:value-of select="format-number(maximum div 1000,'#,###,##0.00')"/>
                </td>
                <td>
                    <xsl:value-of select="format-number(NinetiethPercentile div 1000,'#,###,##0.00')"/>
                </td>
                <td>
                    <xsl:value-of select="format-number(LoadRunnerCount,'#,###,##0')"/>
                </td>
                <td>
                    <xsl:value-of select="format-number(LoadRunnerAverage div 1000,'#,###,##0.00')"/>
                </td>
                <td>
                    <xsl:value-of select="format-number(LoadRunnerMax div 1000,'#,###,##0.00')"/>
                </td>
                <td>
                    <xsl:value-of select="format-number(LoadRunnerNetwork div 1000,'#,###,##0.00')"/>
                </td>
            </tr>
        </tbody>
    </xsl:if>
